...
import { Place, PlacePhoto, LocationCoords, GetNearbyPlacesOptions, CacheData } from '../types';

// Places API specific types
...
interface LegacyPlacesAPIPlace {
  place_id: string;
  name: string;
  vicinity?: string;
  rating?: number;
...
interface NewPlacesAPIResponse {
  places?: NewPlacesAPIPlace[];
}

interface LegacyPlacesAPIResponse {
  status: string;
  results: LegacyPlacesAPIPlace[];
}
...
class PlacesService {
  ...
  private pendingRequests: Map<string, Promise<Place[]>>;
  ...
  async getNearbyInterestingPlaces(options: GetNearbyPlacesOptions): Promise<Place[]> {
    const {
      latitude,
      longitude,
      radius = 5000,
  ...
  private async _fetchNearbyPlaces(
    latitude: number, 
    longitude: number, 
    radius: number, 
    types: string[], 
    ...
    if (config.ENABLE_DEBUG_LOGS) {
      ...
      console.log('🗺️ Process:', typeof process !== 'undefined' ? (process as any).platform : 'undefined');
      console.log('🗺️ Navigator:', typeof (global as any).navigator !== 'undefined' ? (global as any).navigator.userAgent : 'undefined');
    }
    
    // Only use demo data if we're on web AND API keys aren't configured
    const isActuallyWeb = isWeb && typeof (global as any).navigator !== 'undefined' && 
                          (global as any).navigator.userAgent && (!GOOGLE_PLACES_API_KEY || !GOOGLE_MAPS_API_KEY);
  ...
  private async _fetchWithNewPlacesAPI(
    latitude: number, 
    longitude: number, 
    radius: number, 
    types: string[], 
    ...
    const data: NewPlacesAPIResponse = await response.json();
    if (config.ENABLE_DEBUG_LOGS && data.places?.length && data.places.length > 0) {
    ...
    if (formattedResults.length === 0 && validTypes.length > 0) {
      ...
      const broadRequestBody: PlacesRequestBody = {
        maxResultCount: Math.max(1, Math.min(maxResults, 20)), // Ensure between 1-20 as required by API
      ...
      if (broadResponse.ok) {
        const broadData: NewPlacesAPIResponse = await broadResponse.json();
  ...
  private async _fetchWithLegacyAPI(
    latitude: number, 
    longitude: number, 
    radius: number, 
    types: string[], 
    ...
    const data: LegacyPlacesAPIResponse = await response.json();
    if (data.status !== 'OK') {
  ...
  private _formatNewAPIResults(places: NewPlacesAPIPlace[]): Place[] {
  ...
  private _formatLegacyAPIResults(places: LegacyPlacesAPIPlace[]): Place[] {
  ...
  private _formatPhotosFromNewAPI(photos: Array<{ name: string }>): PlacePhoto[] {
  ...
  private _formatPhotosFromLegacyAPI(photos: Array<{ photo_reference: string }>): PlacePhoto[] {
    return photos.slice(0, MAX_PHOTOS).map(photo => ({
  ...
  private async getCachedResult(key: string): Promise<Place[] | null> {
    try {
      const cached = await AsyncStorage.getItem(`places_cache_${key}`);
      if (cached) {
        const { data, timestamp }: CacheData<CompressedPlace[]> = JSON.parse(cached);
        const now = Date.now();
  ...
  private async setCachedResult(key: string, data: Place[]): Promise<void> {
    try {
      const cacheData: CacheData<CompressedPlace[]> = {
        data: this._compressPlacesData(data),
        timestamp: Date.now()
      };
  ...
  async getRandomInterestingPlace(userLocation: UserLocation): Promise<Place | null> {
    try {
  ...
  private _compressPlacesData(places: Place[]): CompressedPlace[] {
  ...
  private _decompressPlacesData(compressedPlaces: CompressedPlace[]): Place[] {
    // Restore full place data structure
    return compressedPlaces.map(place => ({
      ...place,
      types: place.types || [],
      googleMapsUrl: null, // Not stored in compressed format
      openingHours: [], // Not stored in compressed format 
      photos: place.photos || []
    }));
  }
