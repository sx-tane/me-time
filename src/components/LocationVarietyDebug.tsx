...
import { EnhancedSuggestion, LocationSuggestion } from '../types';
...
interface LocationVarietyDebugProps {
  currentSuggestion: EnhancedSuggestion | null;
  previousSuggestion: EnhancedSuggestion | null;
  visible?: boolean;
}

const LocationVarietyDebug: React.FC<LocationVarietyDebugProps> = ({ 
  currentSuggestion, 
  previousSuggestion, 
  visible = false 
}) => {
          ...
          <View style={styles.section}>
            ...
            {currentSuggestion?.locationSuggestions?.slice(0, 3).map((loc, index) => (
              <Text key={index} style={styles.location}>
                {index + 1}. {loc.place?.name || 'Unknown'} ({(loc as any).relevance || 'N/A'})
              </Text>
            ))}
          </View>
