...
export const calculateLocationSimilarity = (locations1: any[], locations2: any[]): number => {
  if (!locations1 || !locations2) return 0;
  
  const names1 = locations1.map((l: any) => l.place?.name?.toLowerCase()).filter(Boolean);
  const names2 = locations2.map((l: any) => l.place?.name?.toLowerCase()).filter(Boolean);
  
  if (names1.length === 0 || names2.length === 0) return 0;
  
  const commonNames = names1.filter((name: string) => names2.includes(name));
  const similarity = (commonNames.length / Math.max(names1.length, names2.length)) * 100;
  
  return similarity;
};

export const getLocationDiversity = (locations: any[]): number => {
...
export const analyzeLocationVariety = (currentLocations: any[], previousLocations: any[]): { 
  similarity: number; 
  diversity: number; 
  varietyScore: number; 
  isVaried: boolean; 
