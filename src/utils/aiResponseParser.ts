...
interface AITask {
  ...
  description: string;
  timeEstimate: string;
  category: SuggestionType | string;
  icon: string;
}

type ValidCategories = Record<SuggestionType | string, string>;

// Default task structure
const DEFAULT_TASK: AITask = {
...
const VALID_CATEGORIES: ValidCategories = {
  mindful: 'heart',
  sensory: 'flower',
  movement: 'walk',
  reflection: 'book',
 ...
 */
const isValidTimeEstimate = (time: any): boolean => {
 ...
 */
const sanitizeTask = (task: any): AITask => {
...
const extractJSONFromResponse = (responseText: string): any => {
  ...
  if (jsonMatch) {
    try {
      ...
      console.warn('Failed to parse JSON from text:', (e as Error).message);
    }
  }
  ...
  try {
    ...
    console.warn('Failed to parse entire response as JSON:', (e as Error).message);
  }
 ...
 */
export const parseAIResponse = (aiResponse: string): any => {
 ...
 */
export const parseMultipleAIResponses = (aiResponse: string, maxTasks: number = 5): any[] => {
    ...
    for (const block of jsonBlocks) {
      ...
      if (match && match[1]) {
        try {
          ...
          console.warn('Failed to parse JSON block:', (e as Error).message);
        }
      }
    }
